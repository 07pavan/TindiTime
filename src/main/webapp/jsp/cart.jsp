...
<body class="d-flex flex-column min-vh-100 bg-light" id="body-cart">
                        ...
                        <c:forEach var="item" items="${cartItems}">
                            <div class="row align-items-center border-bottom pb-3 g-2">
                                ...
                                <div class="col-9 col-sm-4">
                                    <h6 class="fw-bold mb-1 text-dark text-truncate"><c:out value="${item.menuItem.name}" /></h6>
                                    <span class="fs-8 text-muted fw-bold">₹<c:out value="${item.menuItem.price}" /> each</span>
                                </div>
                                <div class="col-6 col-sm-3 d-flex justify-content-center">
                                    ...
                                    <form action="${pageContext.request.contextPath}/cart" method="POST" class="d-inline">
                                        ...
                                        <div class="d-flex align-items-center gap-1 border rounded-pill bg-white p-1">
                                            ...
                                            <span class="font-mono px-2 fw-bold text-dark fs-7"><c:out value="${item.quantity}" /></span>
                                ...
                                <div class="col-4 col-sm-2 text-sm-end text-start">
                                    <span class="fw-bold text-dark font-mono">₹<c:out value="${item.subtotal}" /></span>
                                </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 p-4" id="cart-receipt-card">
                    ...
                    <div class="d-flex flex-column gap-3 border-bottom pb-3 mb-3 fs-7" id="cart-billing-breakdown">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <span class="fw-medium text-dark font-mono" id="val-subtotal">₹<c:out value="${subtotal}" /></span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="text-muted">Promo Discount</span>
                            <span class="fw-medium text-success font-mono" id="val-discount">-₹<c:out value="${discount}" /></span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="text-muted">Delivery partner fee</span>
                            <span class="fw-medium text-dark font-mono" id="val-delivery">₹<c:out value="${deliveryFee}" /></span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="text-muted">Convenience & Restaurant taxes</span>
                            <span class="fw-medium text-dark font-mono" id="val-tax">₹<c:out value="${taxFee}" /></span>
                        </div>
                    </div>
    ...
    <script>
        (function() {
            ...
            // Sync cookie count
            const count = <c:out value="${not empty sessionScope.cartSize ? sessionScope.cartSize : 0}" />;
